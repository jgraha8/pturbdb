...
int main ( int argc, char *argv[]) 
{
  ...
  for (int n=0; n<nproc; n++) {
    if( n == rank ) {
      ...
      cout << "Number of local data elements : " << u->getSizeLocal() << endl;
      cout << "Number of operation data elements : " << u->getSizeOperation() << endl;
      cout << "MPI Topology : " << decomp_dims[0] << "x" << decomp_dims[1] << "x" << decomp_dims[2] << endl;
      cout << endl;
      cout << "DB time range : " << u->getDBTimeMin() << "-" << u->getDBTimeMax() << endl;
  ...
  for (int n=0; n<nproc; n++) {
	...
	cout << "rank, i, x[i]" << rank << " " << i << " " << x[i] << endl;
      }
    }
  }
